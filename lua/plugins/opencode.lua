return {
  {
    ...
    config = function()
      require("opencode").setup({
        ...
        keymap_prefix = "<leader>a", -- Default keymap prefix for global keymaps change to your preferred prefix and it will be applied to all keymaps starting with <leader>a
        ...
        keymap = {
          editor = {
            ["<leader>ag"] = { "toggle", desc = " toggle/close opencode panel" }, -- Open opencode. Close if opened
            ["<leader>ai"] = { "open_input", desc = " open input window" }, -- Opens and focuses on input window on insert mode
            ["<leader>aI"] = { "open_input_new_session", desc = " open input with new session" }, -- Opens and focuses on input window on insert mode. Creates a new session
            ["<leader>ao"] = { "open_output", desc = " open output window" }, -- Opens and focuses on output window
            ["<leader>at"] = { "toggle_focus", desc = " toggle focus between panels" }, -- Toggle focus between opencode and last window
            ["<leader>aT"] = { "timeline", desc = " open timeline picker" }, -- Display timeline picker to navigate/undo/redo/fork messages
            ["<leader>aq"] = { "close", desc = " close opencode UI" }, -- Close UI windows
            ["<leader>as"] = { "select_session", desc = " select session" }, -- Select and load a opencode session
            ["<leader>aR"] = { "rename_session", desc = " rename session" }, -- Rename current session
            ["<leader>ap"] = { "configure_provider", desc = " configure provider/model" }, -- Quick provider and model switch from predefined list
            ["<leader>aV"] = { "configure_variant", desc = " switch model variant" }, -- Switch model variant for the current model
            ["<leader>ay"] = { "add_visual_selection", mode = { "v" }, desc = " add visual selection" },
            ["<leader>az"] = { "toggle_zoom", desc = " toggle zoom" }, -- Zoom in/out on the Opencode windows
            ["<leader>av"] = { "paste_image", desc = " paste image from clipboard" }, -- Paste image from clipboard into current session
            ["<leader>ad"] = { "diff_open", desc = " open diff view" }, -- Opens a diff tab of a modified file since the last opencode prompt
            ["<leader>a]"] = { "diff_next", desc = " next diff" }, -- Navigate to next file diff
            ["<leader>a["] = { "diff_prev", desc = " previous diff" }, -- Navigate to previous file diff
            ["<leader>ac"] = { "diff_close", desc = " close diff view" }, -- Close diff view tab and return to normal editing
            ["<leader>ara"] = { "diff_revert_all_last_prompt", desc = " revert all changes last prompt" }, -- Revert all file changes since the last opencode prompt
            ["<leader>art"] = { "diff_revert_this_last_prompt", desc = " revert current file last prompt" }, -- Revert current file changes since the last opencode prompt
            ["<leader>arA"] = { "diff_revert_all", desc = " revert all changes session" }, -- Revert all file changes since the last opencode session
            ["<leader>arT"] = { "diff_revert_this", desc = " revert current file session" }, -- Revert current file changes since the last opencode session
            ["<leader>arr"] = { "diff_restore_snapshot_file", desc = " restore file snapshot" }, -- Restore a file to a restore point
            ["<leader>arR"] = { "diff_restore_snapshot_all", desc = " restore all snapshots" }, -- Restore all files to a restore point
            ["<leader>ax"] = { "swap_position", desc = " swap panel position" }, -- Swap Opencode pane left/right
            ["<leader>att"] = { "toggle_tool_output", desc = " toggle tools output" }, -- Toggle tools output (diffs, cmd output, etc.)
            ["<leader>atr"] = { "toggle_reasoning_output", desc = " toggle reasoning output" }, -- Toggle reasoning output (thinking steps)
            ["<leader>a/"] = { "quick_chat", mode = { "n", "x" }, desc = " quick chat" }, -- Open quick chat input with selection context in visual mode or current line context in normal mode
          },
          input_window = {
